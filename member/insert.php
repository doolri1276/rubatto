<?php 
	...
	//member_form.php로부터 post로 전달된 값들 받기
	$id = $_POST['id'];
	$pass = $_POST['pass'];
	$name = $_POST['name'];
	$nick = $_POST['nick'];
	...
	if($rowNum){//갯수가 0(false)가 아니면..
		//중복되었다..
		//경고창 띄우고 이전 페이지(member_form.php)로 이동
		echo ("
			<script>
	...
	//위에서 받아온 값들을 member테이블에 insert
	$sql ="INSERT INTO member(id, pass, name, nick, hp, email, regist_day, level) ";
	$sql.="values('$id',password('$pass'),'$name','$nick','$hp','$email','$regist_day','$level')";
