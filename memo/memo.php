		...
		<div id="content">
			...
			<section id="col2">
						...
						<div id="memo_nick"> ▷ 
							<?
								if($userid){
									...
									echo "<a href='../login/login_form.php' id='login_move'>로그인 하러 가기</a>";
								}
							?> 
						</div>
				...
				<?
					...
					//DB의 memo테이블에 저장되있는 글 가져오기
					require "../lib/dbconn.php";

					//읽어오는 데이터를 utf8로 읽도록.
					//mysqli_query($conn, "set names utf8"); lib에 이미 저장해둠.

					//전체 저장 데이터들(레코드row들) 읽어오기
					...
					//총 페이지 수(한 페이지에 5개의 메모글)
					//(1~5 : 1page, 6~10 : 2page, 11~15 : 3page...)
					$pageNum=ceil($rowNum/$scale);
					if($pageNum==0) $pageNum=1;//게시글 0개면 1page

					//페이지 수가 많기에 현재 페이지가 몇페이지 인지. 알 필요가 잇음.
					//이 html의 마지막에 페이지 번호가 있고 이를 선택했을 때 
					//page값을 GET방식으로 memo.php?page=# 다시 로드.
					$page = ($_GET['page'])?$_GET['page']:1;

					//현재 페이지($page)에서 보여줄 5개($scale)의 메모글 출력하기.
					//$result에서 특정 row로 
					//커서를 이동시킬 위치(읽어올 시작 위치) 계산하기.
					//(page 1 - 시작번호 0, page 2 - 시작번호 5, 4page는 시작번호 15)
					//DB 테이블의 row번호는 0번부터임..
					$start = ($page-1)*$scale;

					//$rowNum : 총 메모글 수, $pageNum : 총 페이지 수, $page : 현재 페이지 번호, $start : 현재페이지에서 읽어들일 row의 시작번호
					for($i=$start ; $i<($start+$scale) && $i<$rowNum ; $i++){
						//해당하는 row의 위치로 커서를 이동
						mysqli_data_seek($result,$i);

						//이동한 row에서 레코드 값 읽어오기
							...
							<div class="reply">
								...
								<div class="reply_area">
									...
									<div class="reply_list">
										<!-- 현재 메모글에 해당하는 댓글들을 memo_ripple 테이블에서 읽어오기 -->
										<?
											//memo_ripple테이블에서 현재 메모글의 댓글들(parent필드값을 확인)
														...
														<ul>
															...
															<li class="list_title4">
																...
																if($reply_id==$userid || $userid=="admin"){
																	echo "<a href='delete_reply.php?num=$reply_num&page=$page'>삭제<a>";
																}
									...
									<div class="reply_write">
										...
										<form action="insert_reply.php" method="post">
											...
											<input type="image" name="image" src="../img/memo_ripple_button.png">
											<!-- 메모 글 번호 전송이 필요하여.. -->
				...
				<div id="page_num">
					...
					<?
						...
						if($page<$pageNum){
							?>
								&nbsp;&nbsp;<a href="memo.php?page=<?=$page+1?>">다음 ▶</a>
								&nbsp;&nbsp;<a href="memo.php?page=<?=$pageNum?>">맨끝 ▶</a>
							<?
						}else{
							echo "&nbsp;&nbsp;다음 ▷&nbsp;&nbsp;맨끝 ▷";
						}
